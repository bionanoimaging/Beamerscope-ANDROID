...
import static de.beamerscope.nativepart.NativePart.getAngleVector;
import static de.beamerscope.nativepart.NativePart.getFFTVector;
...
public class CreatePatterns {
    ...
    public static double[] generateOptNumVect(Mat complexInput){
        // reshape complexInput into Vector to pass to the NN

        //detectDigit(outputMat.getNativeObjAddr(), illpattern.getNativeObjAddr());

        Mat outputMat = new Mat();
        getFFTVector(complexInput.getNativeObjAddr(), outputMat.getNativeObjAddr());

        // Create Double Array from Mat
        MatOfDouble doubleMat = new MatOfDouble(CvType.CV_64F);
        outputMat.convertTo(doubleMat,CvType.CV_64F);
        double[] doubleArray = doubleMat.toArray();

        return doubleArray;
    }

    public static Mat generateOptNumMat(Mat complexInput){
        // reshape complexInput into Vector to pass to the NN
        ...
        getAngleVector(complexInput.getNativeObjAddr(), outputMat.getNativeObjAddr());
        //getFFTVector(complexInput.getNativeObjAddr(), outputMat.getNativeObjAddr());
        return outputMat;
    }
